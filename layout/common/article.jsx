...
module.exports = class extends Component {
    render() {
        ...
        // Check if Disqus is enabled for comment count
        const disqusEnabled = config.comment && config.comment.type === 'disqus' && config.comment.shortname;
        const disqusThread = disqusEnabled ? '#disqus_thread' : '';
        ...
        return <Fragment>
            ...
            <div class="card">
                ...
                {cover ? <div class="card-image">
                    {index ? <a href={url_for(page.link || page.path) + disqusThread} class="image is-7by3">
                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`} role="article">
                    ...
                    {page.layout !== 'page' ? <div class="article-meta is-size-7 is-uppercase level is-mobile">
                        <div class="level-left">
                            ...
                            {/* Disqus comment count - show only on index page */}
                            {index && disqusEnabled ? <span class="level-item">
                                <a href={url_for(page.link || page.path) + disqusThread} class="link-muted">
                    ...
                    {/* Title */}
                    {page.title !== '' && index ? <p class="title is-3 is-size-4-mobile">
                        <a class="link-muted" href={url_for(page.link || page.path) + disqusThread}>{page.title}</a>
                    </p> : null}
                    ...
                    {/* "Read more" button */}
                    {index && page.excerpt ? <a class="article-more button is-small is-size-7" href={`${url_for(page.link || page.path)}${disqusThread}#more`}>{__('article.more')}</a> : null}
                    {/* Share button */}
